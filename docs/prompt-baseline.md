...
1. 官方正式数据字段名是否与 `docs/data-schema.md` 的 `idx/title/author/content/qa_words/qa_sents/choose` 完全一致。
2. `ans_qa_words` 和 `ans_qa_sents` 遇到重复词语或重复句子时，是否按 `docs/data-schema.md` 约定合并为一个 key，还是官方评测按位置计分。
3. 目标词语或目标句子为空时，输出空对象还是省略字段。
4. 情感选项不足 4 个、超过 4 个或缺失时，`choose_id` 的合法集合如何定义。
5. 词义分和翻译分使用人工评分、规则匹配、LLM judge，还是官方评测脚本。
6. 总分的权重是否由官方规则确定；若无官方权重，内部 baseline 是否需要固定临时权重。
7. P14 / P8 / P4 的具体模型名称、量化方式和推理后端是否需要写入实验 ID。
8. few-shot 样例池是否允许使用开发集答案，还是必须仅使用训练集答案。
9. baseline 主结果是否严格禁止 retry；如果允许 retry，是否作为独立实验记录。
