...
本文定义 `BCD0`、`BCD1`、`BCD2` 与 `BCD-H` 的最小可执行设计。BCD 是后续结构消融，不是主路线；只有 `BC8-final`、`H1/H2` harness 结果和 `docs/eval-plan.md` 的评测协议稳定后才启动。
...
`BCD1` 是主结构消融，实验命名沿用 `plan.md` / `docs/eval-plan.md` 的 `BCD1`。
...
BCD 实验记录复用 `docs/eval-plan.md` 字段，并补充：
