...
# ============================================================
SERVER = "swh@101.6.42.125"                      # SSH user@host
REMOTE_DIR = "/home/swh/AI/Experiment/CCL25_eval/"           # Remote project root
...
def get_local_worktree_context() -> dict[str, str]:
...
def print_worktree_sync_context() -> None:
    context = get_local_worktree_context()
    print(f"[Local worktree] {context['root']}")
    print(f"[Local branch] {context['branch']} ({context['commit']})")
    print(f"[Remote cwd] {SERVER}:{REMOTE_DIR}")
    print("[Sync guard] Ensure this worktree's sync.sh is the only sync process targeting the remote cwd.")
